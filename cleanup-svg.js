...
function cleanupIcons(src, dest) {
...
fs.readdir(base, function(err, items) {
	if (err) {
		return console.log("Readdir error: " + err.message);
	}

	items.forEach(function(item) {
		if (["cleanup-svg.js", "node_modules"].indexOf(item) !== -1) {
			return;
		}

		fs.stat(path.join(base, item), function(err, stat) {
			if (stat.isDirectory()) {
				var src = path.join(base, item);
				var dest = path.normalize(path.join(base, "..", item, "client/public/icons"));

				cleanupIcons(src, dest);
			}
		});
	});
});
